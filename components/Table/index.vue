    ...
    <div class="table w-4/5 mx-auto">
      <div class="table-header-group pb-10">
        <div class="table-row bg-gray-500/50 bg-opacity-10">
          <div class="table-cell px-5 font-bold pl-5 p-2 text-center relative" v-for="item in items" :key="item" @click="sortTodos(item.value)" :class="changeSortIcon(item.value)">
            {{ item.text }}
          </div>
        </div>
      </div>
